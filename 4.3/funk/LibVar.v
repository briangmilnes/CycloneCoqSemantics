...
Module Type Variables.
  Parameter var : Set.
  Parameter lt  : var -> var -> bool.
  Parameter var_gen_list : list var -> var.
  Axiom lt_irr : forall v,  not (lt v v).
(*
  Axiom var_inhab : Inhab var.
  Parameter var_comp : Comparable var.
  Instance var_comparable : Comparable var := var_comp.
*)
  Axiom var_gen_list_spec : forall (n : var) l,
      n \in from_list l -> 
            lt n (var_gen_list l).
End Variables.
...
Module NatVariables.
  Definition var := nat.
  Definition lt  := Nat.leb.
  Lemma lt_irr: forall v, not (lt v v).
  Admitted.
  ...
  Lemma var_gen_list_spec : forall (n : var) l,
      n \in from_list l -> 
            lt n (var_gen_list l).
  Proof using.
    unfold lt, var.
...
Module VariablesFun(v : Variables) : VariablesType.

 Definition var := v.var.
 Definition vars := fset var.
 Definition lt   := v.lt.
 Definition var_gen_list := v.var_gen_list.
 Definition lt_irr := v.lt_irr.
 (*Definition var_inhab := v.var_inhab.
 Definition var_comp : v.var_comp.
 Definition var_comparable : v.var_comparable.
*)
 ...
 Proof using.
   ...
   forwards M: v.var_gen_list_spec H. 
   apply lt_irr in M.
   false.
 Qed.
...
Module NatVar := VariablesFun(NatVariables).
