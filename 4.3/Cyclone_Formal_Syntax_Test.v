...
Example get_Upsilon_abstract_unknown:
  forall vp vp',
  LVPE.V.get vp (LVPE.V.concat (LVPE.V.single vp' tau) udot)  = Some tau.
Proof.
  simpl_get.
  try case_varpath.
  admit.
...
Example get_Upsilon_abstract_neq:
  forall vp vp',
    vp <> vp' ->
    LVPE.V.get vp (LVPE.V.concat (LVPE.V.single vp' tau) udot)  = None.
Proof.
  simpl_get.
  case_varpath~.
Qed.
...
Example get_Upsilon_pair_abstract:
  forall x x' p p',
    LVPE.V.get (x,p) (LVPE.V.concat (LVPE.V.single (x',p') tau) udot)  = Some tau.
Proof.
  simpl_get.
  case_varpath~.
  admit.
...
Example getUx_abstract_neq_l:
  forall x p x',
    x <> x' ->
    LVPE.V.get (x,p) (LVPE.V.concat (LVPE.V.single (x',p) tau) udot) = None.
Proof.
  simpl_get.
  case_varpath~.
Qed.
