...
Require Export Cyclone_Dynamic_Semantics Cyclone_Static_Semantics_Kinding_And_Context_Well_Formedness.
...
Lemma can_AK_alpha :
  ~ exists (k1 k2 : Kappa),
      AK ((alpha_ ~ k1) & ddot) (ftvar alpha_) k2.
Proof.
  unfold not.
  intros.
...
Lemma can_AK_alpha_A :
  exists (d : Delta) (alpha : var) (k1 k2 : Kappa),
    K ((alpha_ ~ k1) & ddot) (ftvar alpha_) k2.
Proof.
  apply ex_intro with (x:= ddot).
  apply ex_intro with (x:= varx).
  apply ex_intro with (x:= B).
