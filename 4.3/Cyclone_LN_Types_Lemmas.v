...
(* Attempt at a stronger version to make inductions work in TTMP. *)
Lemma open_close_on_open_var :
  forall alpha beta z t i j,
    i <> j ->
    beta <> alpha ->
    beta \notin (fv t) ->
    open_rec i (ftvar beta) (open_rec j z (close_rec j alpha t))
  = open_rec j z (close_rec j alpha (open_rec i (ftvar beta) t)).
Proof.
  induction t; try solve[simpl; intros; try solve [ fequals~ ]];

  intros; simpl; try case_nat~; try case_nat~; simpl; try case_nat~; try case_nat~.
  try solve[intros; simpl; try case_nat~; try case_nat~; simpl; try case_nat~; try case_nat~].
  intros; simpl; try case_var~; simpl; try case_nat~.
  assert(beta \notin fv z).
  auto.
