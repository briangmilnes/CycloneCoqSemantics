...
Require Export LibPath LibVarPath LibVarPathEnv.
...
Hint Extern 3 (Value _) => constructor~.

Definition Heap  := env E.
Definition hdot  : Heap := empty.
Hint Unfold hdot.

Definition State := prod Heap St.  

Definition Delta := env Kappa.
Definition ddot  : Delta := empty.
Hint Unfold ddot.

Definition Gamma := env Tau.
Definition gdot  : Gamma := empty.
Hint Unfold gdot.

Definition Upsilon := LVPE.varpathenv Tau.
Definition udot    : Upsilon := LVPE.V.empty.
Hint Unfold udot.
