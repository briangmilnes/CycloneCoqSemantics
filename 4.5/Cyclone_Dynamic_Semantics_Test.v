...
Example S_Let_3_1_test :
  S empty (letx v s) ((varx ~ v) & empty) s.
Proof.
  unfold_defs.
  apply~ S_let_3_1.
Qed.
...
Example R_assign_3_2_test:
  R  empty                           (e_s (assign (p_e (fevar varx) nil) (i_e (i_i 3))))
    (empty & (varx ~ (i_e (i_i 3)))) (e_s (i_e (i_i 3))).
Proof.
  unfold_defs.
  auto.
  (* eapply R_assign_3_2 *)
...
Example R_star_3_3_test:
  R (empty & (varx ~ v)) (e_s (star (amp (p_e x nil))))
    (empty & (varx ~ v)) (e_s (p_e x nil)).
Proof.
  unfold_defs.
...
Example R_dot_3_4_0_test:
  R empty (e_s (dot (cpair v0 v1) zero_pe))
    empty (e_s v0).
Proof.
  unfold_defs.
...
Example R_dot_3_4_1_test:
  R empty (e_s (dot (cpair v0 v1) one_pe))
    empty (e_s v1).
Proof.
  unfold_defs.
...
Example R_appl_3_5_test:
  R empty (e_s (appl (f_e (dfun tau tau' s)) v))
    empty (e_s (call (letx v s))).
Proof.
  unfold_defs.
...
Example R_call_3_8_test:
  R empty (e_s (call s)) empty (e_s (call s')).
Proof.
  unfold_defs.
...
Example R_amp_3_9_1_test:
  R ((varx ~ v) & empty) (e_s (amp (dot (p_e x nil) zero_pe)))
    ((varx ~ v) & empty) (e_s (amp (p_e x (cons (i_pe zero_pe) nil)))).
Proof.
  unfold_defs.
...
Example R_assign_3_9_2_test:
  R empty (e_s (assign (star (amp (p_e x nil))) v0)) 
    empty (e_s (assign (p_e x nil) v0)).
Proof.
  unfold_defs.
...
Example R_star_3_10_1_test:
  R empty (e_s (star (star (amp (p_e x nil)))))
    empty (e_s (star (p_e x nil))).
Proof.
  unfold_defs.
...
Example R_dot_3_10_2_test:
  R empty (e_s (dot (star (amp (p_e x nil)))  zero_pe))
    empty (e_s (dot (p_e x nil)               zero_pe)).
Proof.
  unfold_defs.
...
Example L_ei_3_4_test:
  L empty (e_s (dot (dot (p_e x nil) zero_pe) zero_pe))
    empty (e_s (dot (p_e x (cons (i_pe zero_pe) nil)) zero_pe)).
Proof.
 unfold_defs.
 auto. (* apply L_ei_3_4.*)
Qed.
