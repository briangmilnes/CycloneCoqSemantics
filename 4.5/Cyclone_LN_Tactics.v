...
Require Export TLC.LibNat TLC.LibVar TLC.LibTactics TLC.LibEnv.
...
(* Turn on and off tracing for Ltacs. *)
...
Hint Extern 1 (_ < _) => idtac "(_ < _)"; trace_goal; nat_math.
Hint Extern 1 (_ <= _) => idtac "(_ <= _)"; trace_goal; nat_math.


(* Modules seems to mess with Auto's definitions. *)
Hint Extern 1 (lt _ _) => idtac "(lt _ _)"; trace_goal; nat_math.
Hint Extern 1 (le _ _ ) => idtac "(le _ _ )"; trace_goal; nat_math.
...
Ltac apply_fresh_base_simple_plus_type lemma gather type :=
  let L0 := gather_vars in let L' := beautify_fset L0 in
   apply @lemma with (L:= L') (tau':= type).

Ltac apply_fresh_base_plus_type lemma gather var_name type:=
  apply_fresh_base_simple_plus_type lemma gather type;
  try (match goal with
    | |- forall _:_, _ \notin _ -> _ => intros_fresh var_name
    | |- forall _:_, fresh _ _ _ -> _ => intros_fresh var_name
       end).

Ltac apply_fresh_base_simple_plus_type_kind lemma gather type kind :=
  let L0 := gather_vars in let L' := beautify_fset L0 in
   apply @lemma with (L:= L') (tau':= type) (k:=kind).

Ltac apply_fresh_base_plus_type_kind lemma gather var_name type kind:=
  apply_fresh_base_simple_plus_type_kind lemma gather type kind;
  try (match goal with
    | |- forall _:_, _ \notin _ -> _ => intros_fresh var_name
    | |- forall _:_, fresh _ _ _ -> _ => intros_fresh var_name
    end).

  Tactic Notation "apply_fresh" constr(T) "with" "type" constr(t') :=
    apply_fresh_base_plus_type T gather_vars ltac_no_arg t'.

  Tactic Notation "apply_fresh" constr(T) "with" "type" constr(t') "kind" constr(k') :=
    apply_fresh_base_plus_type_kind T gather_vars ltac_no_arg t' k'.

Ltac simpl_open_rec :=
  idtac "simpl_open_rec";
  trace_goal;
  timeout 2 
...
Ltac simpl_contexts :=
  ...
  repeat
    match goal with
    | H: context[(fv_heap empty)] |- _ => trace_goal;
             rewrite fv_heap_empty in H
    | |- context[(fv_heap empty)] => trace_goal;
                                    rewrite fv_heap_empty
                                            
    | H: context[(fv_delta empty)] |- _ => trace_goal;
                                          rewrite fv_delta_empty in H
    | |- context[(fv_delta empty)] => trace_goal;
                                     rewrite fv_delta_empty
                                             
    | H: context[(fv_gamma empty)] |- _ => trace_goal;
                                          rewrite fv_gamma_empty in H
    | |- context[(fv_gamma empty)] => trace_goal;
                                     rewrite fv_gamma_empty
                                             
    | H: context[(fv_upsilon LVPE.V.empty)] |- _ => trace_goal;
                                            rewrite fv_upsilon_empty in H
    | |- context[(fv_upsilon LVPE.V.empty)] => trace_goal;
    ...
    | H: context[(fv_upsilon _)] |- _ => trace_goal; unfold fv_upsilon in H
    | |- context[(fv_upsilon _ )] => trace_goal; unfold fv_upsilon

    | H: context[(fv_delta _)] |- _ => trace_goal; unfold fv_delta in H
    | |- context[(fv_delta _ )] => trace_goal; unfold fv_delta

    | H: context[(fv_gamma _)] |- _ => trace_goal; unfold fv_gamma in H
    | |- context[(fv_gamma _ )] => trace_goal; unfold fv_gamma

    | H: context[(fv_heap _)] |- _ => trace_goal; unfold fv_heap in H
    | |- context[(fv_heap _ )] => trace_goal; unfold fv_heap
end.
...
Hint Extern 4 (get _ _ = _) => idtac "(get _ _ = _)"; trace_goal; simpl_env.
Hint Extern 4 (LVPE.V.get _ _ = _) => idtac "(LVPE.V.get _ _ = _)"; trace_goal; simpl_env. 
Hint Extern 4 (_ # _) => trace_goal; simpl_env.
Hint Extern 4 (_ #p _) => trace_goal; simpl_env.
Hint Extern 4 (_ \notin _) => trace_goal; simpl_env.

Ltac fv_of_static_goal :=
  idtac "fv_of_static_goal";
  trace_goal;
  match goal with
    | |- styp ?d ?u ?g ?s ?t =>
...
Ltac simpl_K :=
  ...
  | |- K _ (T.open_rec _ (_ _) (_ _) ) _              =>
    idtac "K _ (T.open_rec _ _ (_ _))"; trace_goal; simpl

  | |- K _ (T.open_rec _ _ (_ _ _) ) _              =>
    idtac "K _ (T.open_rec _ _ (_ _))"; trace_goal; simpl

  | |- K _ (T.open_rec _ _ (_ _ _ _) ) _              =>
    idtac "K _ (T.open_rec _ _ (_ _))"; trace_goal; simpl
...
Ltac simpl_case_nat :=
  trace_goal; simpls*; try case_nat*; try case_nat*.

Hint Extern 6 (styp _ _ _ (TM.open_rec_st _ _ (_ _)) _) => 
idtac "(styp _ _ _ (TM.open_rec_st _ _ _) _)"; simpl_case_nat.
Hint Extern 6 (styp _ _ _ (TM.open_rec_st _ _ (_ _ _)) _ ) => 
idtac "(styp _ _ _ (TM.open_rec_st _ _ _) _)"; simpl_case_nat.
Hint Extern 6 (styp _ _ _ (TM.open_rec_st _ _ (_ _ _ _)) _) => 
idtac "(styp _ _ _ (TM.open_rec_st _ _ _) _)"; simpl_case_nat.

Hint Extern 6 (rtyp _ _ _ (T.open_rec _ _ (_ _)) _) => 
idtac "(rtyp _ _ _ _ (T.open_rec _ _ _))"; simpl_case_nat.
Hint Extern 6 (rtyp _ _ _ (T.open_rec _ _ (_ _ _)) _) => 
idtac "(rtyp _ _ _ _ (T.open_rec _ _ _))"; simpl_case_nat.
Hint Extern 6 (rtyp _ _ _ (T.open_rec _ _ (_ _ _ _)) _) => 
idtac "(rtyp _ _ _ _ (T.open_rec _ _ _))"; simpl_case_nat.

Hint Extern 6 (rtyp _ _ _ (TTM.open_rec_f _ _ (_ _)) _) => 
idtac "(rtyp _ _ _ (TTM.open_rec_f _ _ _) _)"; simpl_case_nat.
Hint Extern 6 (rtyp _ _ _ (TTM.open_rec_f _ _ (_ _ _)) _) => 
idtac "(rtyp _ _ _ (TTM.open_rec_f _ _ _) _)"; simpl_case_nat.
Hint Extern 6 (rtyp _ _ _ (TTM.open_rec_f _ _ (_ _ _ _)) _) => 
idtac "(rtyp _ _ _ (TTM.open_rec_f _ _ _) _)"; simpl_case_nat.

Hint Extern 6 (rtyp _ _ _ (f_e (TTM.open_rec_f _ _ (_ _))) _)
 => idtac "(rtyp _ _ _ (f_e (TTM.open_rec_f _ _ _)) _)"; simpl_case_nat.
Hint Extern 6 (rtyp _ _ _ (f_e (TTM.open_rec_f _ _ (_ _ _))) _)
 => idtac "(rtyp _ _ _ (f_e (TTM.open_rec_f _ _ _)) _)"; simpl_case_nat.
Hint Extern 6 (rtyp _ _ _ (f_e (TTM.open_rec_f _ _ (_ _ _ _))) _)
 => idtac "(rtyp _ _ _ (f_e (TTM.open_rec_f _ _ _)) _)"; simpl_case_nat.
...
Hint Extern 6 (ASGN ?d (utype _ ?tau))   => idtac "(ASGN ?d (utype _ ?tau))"; trace_goal; 
  apply_fresh_from ASGN_utype with fv_of_kinding_goal;
  try simpl; try case_nat*.
Hint Extern 6 (ASGN ?d (etype _ _ ?tau)) => idtac "(ASGN ?d (etype _ _ ?tau))"; trace_goal;
  apply_fresh_from ASGN_etype with fv_of_kinding_goal;
  try simpl; try case_nat*.
Hint Extern 6 (WFU _) => idtac "(WFU _)"; trace_goal;
  ...
  repeat 
    match goal with
      ...
      | H : WFC _ ?u _ |- LVPE.okp ?u => 
        inversion* H
      | |- (WFDG _ (_ & _ ~ _)) => simpl_contexts; constructor*
      | H: WFC ?d _ ?g |- WFDG ?d ?g => inversion* H

      | |- ok empty => apply get_empty
      | |- ok empty => apply get_empty
      | |- LVPE.okp empty => apply get_empty
  end.
