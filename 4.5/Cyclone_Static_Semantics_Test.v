...
Example p63_1:
  styp empty LVPE.V.empty empty
       (letx (i_e (i_i 0)) (e_s (p_e (bevar 0) nil)))
       cint.
Proof.
  apply_fresh styp_let_3_6; auto.
...
Proof.
  intros.
  apply_fresh styp_let_3_6 with type cint.
Admitted.
...
Example SL_3_1_test:
  ltyp empty LVPE.V.empty (empty & (varx ~ cint))  (p_e (fevar varx) nil) cint.
Proof.
  auto.
Qed.

Example SL_3_2_test:
  ltyp empty LVPE.V.empty (empty & (varx ~ (ptype cint))) (star (p_e (fevar varx) nil)) cint.
Proof.
  auto.
  (* apply SL_3_2. *)
Qed.
...
Example SL_3_3_test:
  ltyp empty LVPE.V.empty (empty & (varx ~ (cross cint cint)))
...
Example SL_3_4_test:
  ltyp empty LVPE.V.empty (empty & (varx ~ (cross cint cint))) (dot (p_e (fevar varx) nil) one_pe) cint.
Proof.
...
Example styp_e_test:
  styp empty LVPE.V.empty empty (e_s e) tau.
Proof.
...
Example styp_return_test:
  styp empty LVPE.V.empty empty (retn e) tau.
Proof.
  unfold_test_utilities.
  auto.
...
Example styp_seq_test:
  styp empty LVPE.V.empty empty (seqx s1 s2) tau.
Proof.
  unfold_test_utilities.
  auto.
  (* apply styp_seq_3_3; *)
...
Example styp_while_test:
  styp empty LVPE.V.empty empty (while e s) tau.
Proof.
  unfold_test_utilities.
  auto.
...
Example styp_if_test:
  styp empty LVPE.V.empty empty (if_s e s1 s2) tau.
Proof.
  unfold_test_utilities.
  auto.
  (* apply styp_if_3_5; *)
...
Example styp_let_test:
  styp empty LVPE.V.empty empty (letx e s) tau.
Proof.
  unfold_test_utilities.
  auto.
  (* apply_fresh_from* styp_let_3_6 with fv_of_static_goal. *)
...
Example K_B_test:
  K ((alpha_ ~ B) & empty) (ftvar alpha_) B.
Proof.
  auto.
  (* apply K_B; auto *)
Qed.
...
Example SR_3_1_test:
  rtyp empty LVPE.V.empty (empty & (varx ~ tau)) (p_e x nil) tau.
Proof.
  unfold tau.
  auto.
...
Example SR_3_2_test:
  rtyp empty LVPE.V.empty (empty & (varx ~ (ptype cint))) (star (p_e x nil)) cint.
Proof.
  auto.
  (* apply SR_3_2;
  auto. *)
...
Example SR_3_3_test:
  rtyp empty LVPE.V.empty empty (dot (cpair (i_e (i_i 0)) (i_e (i_i 1))) zero_pe) cint.
Proof.
  auto.
...
Example SR_3_4_test:
  rtyp empty LVPE.V.empty (empty & (varx ~ (cross cint cint)))
...
Example SR_3_5_test:
  rtyp empty LVPE.V.empty empty (i_e (i_i 0)) cint.
Proof.
  auto.
  (* apply SR_3_5. *)
Qed.
...
Example SR_3_7_test:
  rtyp empty LVPE.V.empty empty (cpair (i_e (i_i 0)) (i_e (i_i 1))) (cross cint cint).
Proof.
  auto.
  (* apply SR_3_7;*)
Qed.
...
Example SR_3_8_test:
  rtyp empty LVPE.V.empty (empty & (varx ~ cint))
...
Example SR_3_11_test_simpl:
  rtyp 
    empty LVPE.V.empty ((varx ~ cint) & empty)
...
Example pack_int_open:
  varx <> vary ->
  styp empty LVPE.V.empty (empty & (varx ~ cint) & (vary ~ cint))
        cint
...
Example let_pack_openstar:
  styp empty LVPE.V.empty ((varx ~ cint) & empty) 
       cint 
...
Proof.
  ...
  (L:= (fv_delta empty \u fv_upsilon LVPE.V.empty) \u fv_gamma (varx ~ cint & empty))
  ...
  case_nat.
  apply styp_openstar_3_8
  with (L:= \{varx} \u (fv_delta empty \u fv_upsilon LVPE.V.empty) \u fv_gamma (varx ~ cint & empty))
         (k:= B)
         (tau':= cint).
  simpl.
  intros.
...
Example SR_3_12_test:
  rtyp empty LVPE.V.empty ((varx ~ cint) & empty)
...
Example htyp_empty_test:
  htyp LVPE.V.empty empty empty empty.
Proof.
  apply htyp_empty;
  auto.
Qed.
...
Example htyp_xv_test:
  htyp LVPE.V.empty empty (empty & (varx ~ v) & empty) (empty & (varx ~ tau) & empty).
Proof.
  unfold_test_utilities.
  (* loss of syntax direction here but we don't mind really as eauto is working. *)
  (* auto. *)
  eapply htyp_xv with (g':= empty);
  auto.
  (* Bug needs empty & empty = empty. *)
Admitted.
...
Example program_test:
  prog empty (retn (i_e (i_i 0))).
Proof.
  auto.
  apply program with (u:= LVPE.V.empty) (g:= empty) (tau:=cint);
  auto.
Qed.
