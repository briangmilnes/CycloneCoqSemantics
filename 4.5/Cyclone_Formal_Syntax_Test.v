...
Hint Extern 1 (get _ _ = _) => idtac "(get _ _ = _)"; trace_goal; auto_env.

Example NotInDomHnil :
  get varx (@empty E)  = None.
Proof.
  auto.
Qed.
...
Example NotInDomHdifferent :
  varx <> vary ->
  get vary ((single varx v0) & empty) = None. 
Proof.
  auto.
Qed.
...
Example NotInDomHsecond :
  vary <> varx ->
  vary <> varz ->
  get vary ((single varx v0) & ((varz ~ v0) & empty)) = None.
Proof.
  auto.
Qed.

Example NotInDomDnil :
  get vary (@empty Tau) = None.
Proof.
  auto.
Qed.

Example NotInDomDsingle :
  get alpha_ ((single alpha_ A) & empty) = Some A.
Proof.
  auto.
Qed.
...
Example NotInDomDdifferent :
  alpha_ <> beta_ ->
  get beta_ ((single alpha_ B) & empty) = None.
Proof.
  auto.
Qed.
...
Example NotInDomDsecond :
  alpha_ <> beta_ ->
  beta_ <> gamma_ ->
   get beta_ ((single alpha_ A) & (single gamma_ B) & empty) = None.
Proof.
  auto.
Qed.
...
Example get_Upsilon_abstract_unknown:
  forall vp vp',
  LVPE.V.get vp (LVPE.V.concat LVPE.V.empty (LVPE.V.single vp' tau))  = Some tau.
Proof.
  intros.
  simpl_env.
  try case_varpath.
...
Example get_Upsilon_abstract_identical:
  forall vp,
    (If vp = vp then Some tau else LVPE.V.get vp LVPE.V.empty) = Some tau.
Proof.
  intros.
  simpl_env.
Qed.
...
Example get_Upsilon_abstract_neq:
  forall vp vp',
    vp <> vp' ->
    LVPE.V.get vp (LVPE.V.concat LVPE.V.empty (LVPE.V.single vp' tau))  = None.
Proof.
  intros.
  simpl_env.
  auto.
...
Example get_Upsilon_pair_abstract:
  forall x x' p p',
    LVPE.V.get (x,p) (LVPE.V.concat LVPE.V.empty (LVPE.V.single (x',p') tau) )  = Some tau.
Proof.
  intros.
  try simpl_env.
  admit.
...
Example getUx_abstract_neq_l:
  forall x p x',
    x <> x' ->
    LVPE.V.get (x,p) (LVPE.V.concat LVPE.V.empty (LVPE.V.single (x',p) tau)) = None.
Proof.
  intros.
  simpl_env.
Qed.
