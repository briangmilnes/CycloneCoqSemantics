...
(* This is the version just for the lemmas about substituting types into terms. *)
...
Proof.
  ...
  pick_fresh alpha;
  apply~ (@open_rec_t_t_lc_ind t0 0 (ftvar alpha));
  apply~ H0.
Qed.
...
Lemma size_t_open_rec_tv_t : forall alpha t,
...
Lemma size_t_close_rec_tv_t : forall alpha t,
...
Lemma lc_t_induct_size :
  forall P : Tau -> Prop,
    (forall alpha,  P (ftvar alpha)) ->
    (P cint) ->
    (forall t1 t2, (lc_t t1) -> (P t1) -> (lc_t t2) -> (P t2) ->
                   P (cross t1 t2)) ->
    (forall t1 t2, (lc_t t1) -> (P t1) -> (lc_t t2) -> (P t2) ->
                   P (arrow t1 t2)) ->
    (forall t1, (lc_t t1) -> (P t1) -> P (ptype t1)) ->
    ...
    (forall k t1,
       ...
       (forall t2 alpha,
          alpha \notin ftv_t t2 ->
          size_t t2 = size_t t1 ->
          lc_t (open_rec_t_t 0 (ftvar alpha) t2) ->
          P (open_rec_t_t 0 (ftvar alpha) t2)) ->
       P (utype k t1)) ->
...
Proof.
  ...
  induction t using (@measure_induction _ size_t).
  introv T. 
  ...
  try introv Fr Eq T.
  apply~ H.
  try rewrite~ size_t_open_rec_tv_t.
