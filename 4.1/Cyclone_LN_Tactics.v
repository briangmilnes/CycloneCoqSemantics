...
Ltac gather_vars :=
  ...
  let C' := gather_vars_with (fun x : Tau => ftv_t x) in
  let D' := gather_vars_with (fun x : V => ftv_tm_v x) in
  let E' := gather_vars_with (fun x : St => ftv_tm_st x) in
  let F' := gather_vars_with (fun x : E => ftv_tm_e x) in
  let G' := gather_vars_with (fun x : F => ftv_tm_f x) in  
  let H' := gather_vars_with (fun x : Term => ftv_tm x) in
  let I' := gather_vars_with (fun x : St => ftv_tm_st x) in
  let J' := gather_vars_with (fun x : E => ftv_tm_e x) in
  let K' := gather_vars_with (fun x : F => ftv_tm_f x) in
  let L' := gather_vars_with (fun x : Term => ftv_tm x) in
