...
Example subst_t_tv_alpha: (subst_t_tv (ftvar alpha) tau alpha) = tau.
Proof.
  repeat case_subst.
Qed.

Example subst_t_tv_beta: (subst_t_tv (ftvar beta) tau alpha) = (ftvar beta).
Proof.
  repeat case_subst.
Qed.

Example subst_t_tv_cint: (subst_t_tv cint tau alpha) = cint.
Proof.
  reflexivity.
Qed.

Example subst_t_tv_cross_none : 
  (subst_t_tv (cross t0 t1) tau alpha) = cross t0 t1.
Proof.
  reflexivity.
Qed.

Example subst_t_tv_partial:
  (subst_t_tv (cross cint cint)  t1 alpha) = cross cint cint.
Proof.
  reflexivity.
Qed.

Example subst_t_tv_cross_0 : 
  (subst_t_tv (cross (ftvar alpha) (ftvar alpha)) cint alpha) = 
   cross cint cint.
Proof.
  repeat case_subst.
Qed.

Example subst_t_tv_cross_l : 
  (subst_t_tv (cross (ftvar alpha) t1) tau alpha) = cross tau t1.
Proof.
  repeat case_subst.
Qed.

Example subst_t_tv_cross_r : 
  (subst_t_tv (cross t0 (ftvar alpha)) tau alpha) = cross t0 tau.
Proof.
  repeat case_subst.
Qed.

Example subst_t_tv_arrow_l: 
  (subst_t_tv (arrow (ftvar alpha) t1) tau alpha) = arrow tau t1.
Proof.
  repeat case_subst.
Qed.

Example subst_t_tv_arrow_l_beta: 
  (subst_t_tv (arrow (ftvar beta) t1) tau alpha) = arrow (ftvar beta) t1.
Proof.
  repeat case_subst.
Qed.

Example subst_t_tv_arrow_r: 
  (subst_t_tv (arrow t0 (ftvar alpha)) tau alpha) = arrow t0 tau.
Proof.
  repeat case_subst.
Qed.

Example subst_t_tv_arrow_r_beta: 
  (subst_t_tv (arrow t0 (ftvar beta)) tau alpha) = arrow t0 (ftvar beta).
Proof.
  repeat case_subst.
Qed.

Example subst_t_tv_ptype_beta: 
  subst_t_tv (ptype (ftvar beta)) tau alpha = (ptype (ftvar beta)).
Proof.
  repeat case_subst.
Qed.

Example subst_t_tv_ptype_alpha: 
  subst_t_tv (ptype (ftvar alpha)) tau alpha = (ptype tau).
Proof.
  repeat case_subst.
Qed.

Example subst_t_tv_utype: 
  (subst_t_tv (utype k cint) tau alpha) = (utype k cint).
Proof.
  repeat case_subst.
Qed.

Example subst_t_tv_utype_alpha: 
  (subst_t_tv (utype k (ftvar alpha)) tau alpha) = (utype k tau).
Proof.
  repeat case_subst.
Qed.

Example subst_t_tv_utype_beta: 
  (subst_t_tv (utype k (ftvar beta)) tau alpha) = 
  (utype k (ftvar beta)).
Proof.
  repeat case_subst.
Qed.

Example subst_t_tv_etype: 
  (subst_t_tv (etype aliases k t0) tau alpha) = 
  (etype aliases k t0).
Proof.
  repeat case_subst.
Qed.

Example subst_t_tv_etype_alpha: 
  (subst_t_tv (etype aliases k (ftvar alpha)) tau alpha) = 
  (etype aliases k tau).
Proof.
  repeat case_subst.
Qed.

Example subst_t_tv_etype_beta:
  (subst_t_tv (etype aliases k (ftvar beta)) tau alpha) = 
...
Example ftv_tm_intcrossint :
  ftv_tm_tau (cross cint cint) = \{}_t.
Proof.
  repeat case_subst.
Qed.

Example ftv_tm_intcrossint_1 :
  ftv_tm_tau (cross (ftvar beta) cint) = \{ beta }_t.
Proof.
  tauto.
Qed.

Example NotFreeInTau_intcrossint_2 :
  ftv_tm_tau (cross cint (ftvar beta)) = \{ beta }_t.
Proof.
  tauto.
Qed.

Example ftv_tm_intarrow :
  ftv_tm_tau (arrow cint cint) = \{}_t.
Proof.
  tauto.
Qed.

Example ftv_tm_intarrow_1:
  ftv_tm_tau (arrow (ftvar beta) cint) =  \{beta}_t.
Proof.
  tauto.
Qed.

Example ftv_tm_intarrow_2:
  ftv_tm_tau (arrow cint (ftvar beta)) = \{beta}_t.
Proof.
  tauto.
Qed.

Example NotFreeInTau_ptype: 
  ftv_tm_tau (ptype cint) = \{}_t.
Proof.
  repeat case_subst.
Qed.

Example ftv_tm_ptype_false:
 ftv_tm_tau (ptype (ftvar beta)) = \{ beta }_t.
Proof.
  tauto.
Qed.

Example ftv_tm_utype_bound:
  ftv_tm_tau (utype A cint) = \{}_t.
Proof.
  repeat case_subst.
Qed.

Example ftv_tm_utype_bound_2:
  ftv_tm_tau (utype A (btvar 0)) = \{}_t.
Proof.
  repeat case_subst.
Qed.

Example ftv_tm_utype_bound_3:
  ftv_tm_tau (utype A (ftvar beta)) = \{ beta }_t.
Proof.
  tauto.
Qed.

Example ftv_tm_etype_bound:
  ftv_tm_tau (etype aliases A cint) = \{}_t.
Proof.
  repeat case_subst.
Qed.

Example ftv_tm_etype_bound_2:
  ftv_tm_tau (etype aliases A (btvar 0)) = \{}_t.
Proof.
  repeat case_subst.
Qed.

Example NotFreeInTau_etype_bound_3:
 ftv_tm_tau (etype aliases A (ftvar beta)) = \{ beta }_t.
Proof.
  tauto.
Qed.
*)
