...
| S_openstar_3_8: forall (tau tau' : Tau) (v v' : E) (q : Phi) (k : Kappa) (alpha : TVar) (x x': EVar) (h : H) (s : St) (p : Path),
                    Value v ->
                    x <> x' ->
...
with R : H -> St -> H -> St -> Prop :=
 | R_get_3_1 : forall (h  : H) (x : EVar) (p : Path) (v v' : E),
                    HM.map h x = Some v' -> 
                    get v' p v ->
                    Value v ->
                    Value v' ->
 ...
 | R_assign_3_2:
     forall (h' h : H) (v v' v'' : E) (x : EVar) (p : Path),
       HM.map h x = Some v' ->
       set v' p v v'' ->
       HM.add h x v'' = h' -> 
       Value v   ->
 ...
 | R_assign_3_10_3: forall (h h' : H) (e e' : E) (x : EVar) (p : Path),
                    R h (e_s e) h' (e_s e') ->
...
with L : H -> St -> H -> St -> Prop :=
 ...
 | L_xpi_3_1: forall (h : H) (x : EVar) (p : Path) (ipe : IPE),
                L h (e_s (dot (p_e x p) ipe))
                  h (e_s (p_e x (p ++ [i_pe ipe])))

 | L_staramp_3_2: forall (h : H) (x : EVar) (p : Path),
