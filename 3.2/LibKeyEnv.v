...
(* ********************************************************************** *)
(** * Definition of a more general environment model and its basic operations *)
 
(* ---------------------------------------------------------------------- *)
(** ** Abstract definitions *)

Parameter key : Set.
Definition Keys (key : Type) := fset key.
Definition kenv (key : Type) (A:Type) := list (key * A).

Module Type KeyEnvOpsSig.

Section Definitions.
Variable A B K: Type.

Parameter empty : kenv K A.
Parameter empty_def : empty = nil.

Parameter single : K -> A -> kenv K A.
Parameter single_def : 
  single = fun x v => (x,v)::nil.

Parameter concat : kenv K A -> kenv K A -> kenv K A.
Parameter concat_def :
  concat = fun E F => F ++ E.

Parameter singles : list K -> list A -> kenv K A.
Parameter singles_def : 
  singles = fun xs vs =>
    fold_right (fun p acc => concat acc (single (fst p) (snd p))) empty (combine xs vs).

(* Shit naming. *)
Parameter keys : kenv K A -> list K.
Parameter keys_def : 
  keys = map fst.

Parameter values : kenv K A -> list A.
Parameter values_def :
  values = map snd.

Parameter fold_vars : (A -> vars) -> kenv K A -> vars.
Parameter fold_vars_defs : 
  fold_vars = fun fv E =>
    fold_right (fun v acc => fv v \u acc) \{} (values E).

Parameter dom : kenv K A -> Keys K.
Parameter dom_def : 
  dom = fold_right (fun p  (E : fset K)   => \{fst p} \u E) \{}.

Parameter map : (A -> B) -> kenv K A -> kenv K B.
Parameter map_def : 
  map = fun f E => 
    LibList.map (fun p => (fst p, f (snd p))) E.

Parameter map_keys : (K -> K) -> kenv K A -> kenv K A.
Parameter map_keys_def : 
  map_keys = fun f E =>
    LibList.map (fun p => (f (fst p), snd p)) E.

Parameter get : K -> kenv K A -> option A.
Fixpoint get_impl (k : K) (E : kenv K A) {struct E} : option A :=
  match E with
  | nil => None
...
Module Export KeyEnvOps : KeyEnvOpsSig.

Section Concrete.
Variable A B K : Type.

Definition empty : kenv K A := nil.
Lemma empty_def : empty = nil.
Proof. reflexivity. Qed.

Definition single x v : kenv K A := (x,v)::nil.
Lemma single_def : 
  single = fun x v => (x,v)::nil.
Proof. reflexivity. Qed.

Definition concat (E F : kenv K A) := F ++ E.
Lemma concat_def : 
  concat = fun E F => F ++ E.
Proof. reflexivity. Qed.

Definition singles (xs : list K) (vs : list A) : kenv K A := 
  fold_right (fun p acc => concat acc (single (fst p) (snd p))) empty (combine xs vs).
Lemma singles_def : 
  singles = fun xs vs =>
   fold_right (fun p acc => concat acc (single (fst p) (snd p))) empty (combine xs vs).
...
Definition keys : kenv K A -> list K :=
  map fst.
Lemma keys_def : 
  keys = map fst.
Proof. reflexivity. Qed.

Definition values : kenv K A -> list A :=
  map snd.
Lemma values_def : 
  values = map snd.
Proof. reflexivity. Qed.

Definition fold_vars (fv : A -> Keys key) (E : kenv K A) :=
  fold_right (fun v acc => fv v \u acc) \{} (values E).
Lemma fold_vars_defs : 
  fold_vars = fun fv E => fold_right (fun v acc => fv v \u acc) \{} (values E).
Proof. reflexivity. Qed.

Definition map (f:A->B) (E:kenv K A) :=
  LibList.map (fun p => (fst p, f (snd p))) E.
Lemma map_def : 
  map = fun f E => LibList.map (fun p => (fst p, f (snd p))) E.
Proof. reflexivity. Qed.

Definition map_keys (f: key -> key) (E:kenv K A) :=
  LibList.map (fun p => (f (fst p), snd p)) E.
Lemma map_keys_def : 
  map_keys = fun f E => LibList.map (fun p => (f (fst p), snd p)) E.
Proof. reflexivity. Qed.

Definition dom : kenv K A -> vars :=  
  fold_right (fun p E => \{fst p} \u E) \{}.
Lemma dom_def : 
  dom = fold_right (fun p E => \{fst p} \u E) \{}.
Proof. reflexivity. Qed.

Fixpoint get_impl (k : var) (E : kenv K A) {struct E} : option A :=
  match E with
  | nil => None
  | (x,v) :: E' => If k = x then Some v else get_impl k E'
  end.
...
Definition get_or_arbitrary `{Inhab A} x (E:kenv K A) :=
  unsome (get x E).
...
Definition fv_in_values (fv:A->vars) (E:kenv K A) :=
...
Lemma env_ind : forall (P : kenv K A -> Prop),
  (P empty) ->
...
Lemma binds_get_or_arbitrary : forall `{Inhab A} x v (E:kenv K A), 
  binds x v E -> get_or_arbitrary x E = v.
Proof. introv M. unfold get_or_arbitrary. rewrite~ M. Qed.

Definition indom_dec A (E:kenv K A) x :=
  match get x E with None => false | Some _ => true end.
 
Global Instance indom_decidable : forall A (E:kenv K A) x,
